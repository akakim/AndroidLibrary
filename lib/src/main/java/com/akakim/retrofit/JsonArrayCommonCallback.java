...
/**
 * JsonArrayCommonCallback.java -> APICallback.java 로 데이터를 이동함 진행할 예정.
 ...
 */
public class JsonArrayCommonCallback implements Callback<JsonArray> {

	APICallback apiCallback;

	CommonConstant.API apiID;
	public JsonArrayCommonCallback(APICallback apiCallback , CommonConstant.API apiID) {
		...
		if (response.code() == 200 ){
			...
			Integer resultCode = response.code();
				if( resultCode == 200){
					...
					apiCallback.onJsonArraySDPError( call, response  );
				}
	 ...
	 * @return
	 */
//	public Response<JsonObject> addAPIID(Response<JsonArray> response , CommonConstant.API id){
//
